...
class WeekMonthChart extends StatefulWidget {
  ...
  const WeekMonthChart({Key? key, required this.habits}) : super(key: key);

  @override
  _WeekMonthChartState createState() => _WeekMonthChartState();
}

class _WeekMonthChartState extends State<WeekMonthChart> with TickerProviderStateMixin {
          ...
          children: [
            Container(
              ...
              child: TabBar(
                ...
                indicator: BorderTabIndicator(indicatorHeight: 36, textScaleFactor: 0.8),
                isScrollable: true,
              ),
            ),
            SizedBox(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      Text(
                          currentChart == 0
                              ? DateUtil.getWeekPeriodString(_now, currentWeekIndex)
                              : DateUtil.getMonthPeriodString(_now, currentMonthIndex),
                          style: AppTheme.appTheme.numHeadline1(
                              textColor: AppTheme.appTheme.grandientColorEnd().withOpacity(0.9),
                              fontWeight: FontWeight.bold,
                              fontSize: 18))
                    ],
                  ),
  ...
  String getMonthByIndex(int index) {
    ...
    DateTime time = DateTime(currentMonthFirstDay.year, currentMonthFirstDay.month, index);
